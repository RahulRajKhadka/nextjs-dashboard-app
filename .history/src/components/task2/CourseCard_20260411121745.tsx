...
export default function CourseCard({
  ...
  useLayoutEffect(() => {
    ...
    // Inside the expand branch (if isExpanded)
...
// TOPBAR ANIMATION - Always from right, no y movement
tl.fromTo(topbarRef.current,
  { opacity: 0, x: 60 },      // Start: invisible, 60px to the right
  { opacity: 1, x: 0, duration: 0.5, ease: "power2.out" },  // End: visible, normal position
  0.3
);

// Icons animation (keep as is)
const iconsFromX = 120;  // Always from right
