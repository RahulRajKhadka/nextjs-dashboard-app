...
export default function CourseCards() {
  ...
  const [directionMap, setDirectionMap] = useState<Map<number, "left" | "right">>(new Map([[1, "right"]]));
  ...
  const handleClick = (courseId: number) => {
    ...
    // Calculate direction based on which card is to the left or right
    ...
    // Animate old label back to collapsed position
    if (oldLabel) {
      tl.to(
        oldLabel,
        {
    ...
    // Update expanded state at mid point
    tl.call(() => {
    ...
    // Animate new label to expanded position beside number
