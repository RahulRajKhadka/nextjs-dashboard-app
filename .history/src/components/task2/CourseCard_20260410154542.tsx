...
export default function CourseCard({
  ...
  useLayoutEffect(() => {
    ...
    if (isExpanded) {
      // Create timeline for smooth coordination
      const tl = gsap.timeline();
      
      // Show expanded view
      gsap.set(expandedViewRef.current, { display: "grid" });

      // Measure label and number positions for target
      ...
      // 2. Label animation: rotate -90° → 0°, move to beside number
      ...
      // 3. Number color change to white
      tl.to(numberRef.current, {
        color: "#ffffff",
        duration: 0.4,
      }, 0.2);

      // 4. Top bar animation (slides in from left/right)
      ...
      // 5. Icons animation (staggered, slide in from sides)
      ...
      // COLLAPSE ANIMATIONS - Reverse everything
      const tl = gsap.timeline();
      ...
      // 2. Label returns to collapsed position (center, rotated -90°)
      ...
      // 3. Number color returns to original
      tl.to(numberRef.current, {
        color: course.textColor,
        duration: 0.3,
      }, 0.1);
  ...
  return (
    ...
    >
      ...
      {/* COLLAPSED STATE - Label and Number stacked vertically */}
      {!isExpanded && (
        <div className="absolute inset-0 z-[3] flex flex-col items-center pointer-events-none py-8">
          ...
          {/* NUMBER - pushed to bottom */}
          <div 
            ref={numberRef}
            className="flex items-center justify-center mt-auto mb-4"
      ...
      {/* EXPANDED STATE - Hidden elements that animate in */}
      {isExpanded && (
        <>
          {/* Label (will be animated by GSAP) */}
          <div 
            ref={collapsedLabelRef}
            className="absolute z-[4] flex flex-col gap-1 pointer-events-none"
            style={{ color: "#ffffff" }}
          >
          ...
          {/* Number (will be animated by GSAP) */}
          <div 
            ref={numberRef}
            className="absolute bottom-0 left-6 z-[3] flex items-center justify-center p-4 pointer-events-none"
            style={{ color: "#ffffff" }}
          >
            <span
              className="font-black leading-none"
      ...
      {/* EXPANDED VIEW - Top bar and icons */}
      <div
        ref={expandedViewRef}
